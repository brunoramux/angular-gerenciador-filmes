...
export class CreateMovie {
  ...
  // signo para criar imagem para mostrar preview na tela
  ...
  onFileSelected(event: Event) {
    ...
    if (input.files && input.files[0]) {
      ...
      // exclui imagens antigas da memoria do navegador, se existir
      if (this.imagePreview()) {
        URL.revokeObjectURL(this.imagePreview()!);
      }

      // cria nova URL com imagem selecionada pelo usuario
      const objectUrl = URL.createObjectURL(file);

      // seta URL no signo imagePreview, que atualiza a interface
